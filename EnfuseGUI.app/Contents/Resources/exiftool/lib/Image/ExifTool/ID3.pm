...
{
    my ($exifTool, $val) = @_;
    my $from = $exifTool->Options('CharsetID3');
    my $to = $exifTool->Options('Charset');
    return $val if $from eq $to;
    $val = $exifTool->Charset2Unicode($val, 'MM', $from);
    return $exifTool->Unicode2Charset($val, 'MM', $to);
}
